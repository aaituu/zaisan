...
//------------------------------------------------------------------------------------------------------
//===========================================
// PARALLEL STREAM BRIDGE - ДОБАВЬТЕ В КОНЕЦ
//===========================================
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[Content Script Bridge] Got message:', message);

    if (message.type === 'get-streaming-status') {
        console.log('[Content Script Bridge] Sending status:', streamingState.isStreaming);
        sendResponse({
...
window.addEventListener('message', (event) => {
    ...
    console.log('[Content Script Bridge] Got message from page:', event.data.type);

    if (event.data.type === 'parallel-stream-status') {
        streamingState.isStreaming = event.data.isStreaming;
    ...
    if (event.data.type === 'parallel-stream-started') {
        streamingState.isStreaming = true;
        console.log('[Content Script Bridge] Stream STARTED');
    }

    if (event.data.type === 'parallel-stream-stopped') {
        streamingState.isStreaming = false;
        console.log('[Content Script Bridge] Stream STOPPED');
    }

    if (event.data.type === 'parallel-stream-url') {
        chrome.runtime.sendMessage({
            type: 'stream-url',
            url: event.data.url
        }).catch(() => {});
    ...
    if (event.data.type === 'parallel-stream-viewers') {
        chrome.runtime.sendMessage({
            type: 'viewer-count',
    ...
    if (event.data.type === 'parallel-stream-error') {
        chrome.runtime.sendMessage({
            type: 'stream-error',
