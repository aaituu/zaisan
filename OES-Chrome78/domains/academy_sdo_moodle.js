...
function initOes(){
	...
	//--------------------------------------------------
	console.log('Moodle ACADEMY SDO');
	//--------------------------------------------------
...
xhr.open("GET", "https://academy-sdo.oes.kz/extension/moodle-status.php");
xhr.send();
xhr.responseType = "json";
xhr.onload = () => {
  if (xhr.readyState == 4 && xhr.status == 200) {
    var otv = xhr.response; 
    console.log("[OES Extension] Got moodle state data", otv);
...
document.addEventListener('click', function(e) {
	...
	//--------------------
  	window.location.href = "https://academy-sdo.oes.kz/internal_assignment?system=DEFAULT_MOODLE&examName=" + encodeURIComponent(examName) + "&url=" + encodeURIComponent(window.location.href) + "&backurl=" + encodeURIComponent(window.location.href) + "&cookies=" + encodeURIComponent(JSON.stringify(cookieData));
  	return false;
}); 
//------------------------------------------------------
(function(){
