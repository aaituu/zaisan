...
var jqueryLoaded = (function(){
	...
	if (parts[1] == 'courses' && parts[3] == 'quizzes' && parts[5] == 'take'){
		$.get('https://canvas.yu.edu.kz/api/v1/courses/' + parts[2], function(data) {
			var examName = data.name;
			if (examName == null && examName == undefined) return;
			var secondName = $('.quiz-header h1').text().trim();
	...
	if (parts[1] == 'courses' && parts[3] == 'assignments'){
		$.get('https://canvas.yu.edu.kz/api/v1/courses/' + parts[2], function(data) {
			var examName = data.name;
			if (examName == null && examName == undefined) return;
			var secondName = $('.assignment-title h1').text().trim();
			var fullname = examName + ', ' + secondName; 
			console.log('examName', fullname);
