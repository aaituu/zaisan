...
if (
  window.location.hostname.toLowerCase().endsWith(".daryn.onlline") &&
  window.location.hostname.toLowerCase() != "proctoring.daryn.online"
)
  isExternal = true;
if (
  window.location.href
    .toLowerCase()
    .startsWith("https://eqyzmet.gov.kz/proctoring")
)
  isExternal = false;
if (
  window.location.href
    .toLowerCase()
    .startsWith("https://proctoring.daryn.online")
)
  isExternal = false;
if (
  window.location.href.toLowerCase().startsWith("https://proctoring.enbek.kz")
)
  isExternal = false;

if (window.location.hostname.toLowerCase().endsWith(".uchet.kz"))
  isExternal = true;
if (
  window.location.href
    .toLowerCase()
    .startsWith("https://proctoring-competence.uchet.kz")
)
  isExternal = false;
if (window.location.href.toLowerCase().startsWith("https://proctor.crocos.kz"))
  isExternal = false;

if (
  window.location.href.toLowerCase().startsWith("https://aet.astanait.edu.kz")
)
  isExternal = false;

if (
  window.location.href.toLowerCase().startsWith("https://proctoring.icert.kz")
)
  isExternal = false;
//------------------------------------------------------------------------------------------------------
...
if (
  ...
  //--------------------------------------------------------------------------------------------------
  console.log(
    "[OES Extension] Loaded content script from " +
      (isExternal ? "external tab" : "iframe")
  );
  //--------------------------------------------------------------------------------------------------
  ...
  // После строки 95 (после всех injectScript для domains)
  injectScript("injector.js");
  //--------------------------------------------------------------------------------------------------
  // Динамические расширения
  var host = window.location.hostname.toLowerCase();
  if (host == "platonus.almau.edu.kz")
    injectScript("domains/almau_platonus.js");
  ...
  if (host == "platonus.turan-edu.kz")
    injectScript("domains/turan_platonus.js");
  ...
  if (host == "ospanov.c-platonus.kz")
    injectScript("domains/ospanov_platonus.js");
  //--------------------------------------------------------------------------------------------------
  ...
  window.addEventListener("message", function (event) {
    ...
    if (event.data == "get-jquery-url") {
      window.postMessage(
        "jquery-url-" + chrome.runtime.getURL("jquery-3.6.0.js"),
        "*"
      );
    }
  });
  ...
  window.addEventListener("message", function (event) {
    ...
    if (event.data == "get-version") {
      ...
      window.postMessage(
        JSON.stringify({ type: "set-version", version: protocolVersion }),
        "*"
      );
    }
    if (event.data == "get-brand") {
      window.postMessage(
        JSON.stringify({ type: "set-brand", brand: "oes" }),
        "*"
      );
    }
    if (event.source != window)
      try {
        if (!event.source.location.href.startsWith("https://caspidot.kz"))
          return;
      } catch (e) {
        return;
      }
    try {
      ...
      if (data.type == "get-cookie") {
        if (
          !event.source.location.origin.endsWith(".oes.kz") &&
          ...
          !event.source.location.origin.endsWith("icert.kz")
        )
          return;
        port.postMessage(event.data);
        return;
